...
pub fn interpret(src: &str) -> InterpretedData {
    println!("{}", "Interpreting brainf**k code...".fg_rgb::<200, 0, 0>());
...
pub fn to_src(data: InterpretedData) -> String {
    ...
    let output = output.replace(
        "#define DATA_SIZE 0",
        format!("#define DATA_SIZE {}", data.src_as_bytes.len()).as_str(),
    );
    let output = output.replace(
        "#define RAW_DATA {0}",
        format!("#define RAW_DATA {{{}}}", data.src_as_array_str).as_str(),
    );
    let output = {
